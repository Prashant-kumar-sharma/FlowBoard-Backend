...
public class KafkaTopicConfig {
    ...
    @Bean public NewTopic premiumActivated() { return TopicBuilder.name("flowboard.payment.premium.activated").partitions(3).replicas(1).build(); }
    @Bean public NewTopic accountStatusChanged() { return TopicBuilder.name("flowboard.account.status.changed").partitions(3).replicas(1).build(); }
}
