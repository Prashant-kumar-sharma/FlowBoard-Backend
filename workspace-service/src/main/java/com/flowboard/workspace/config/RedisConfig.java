...
public class RedisConfig {
    private static final List<String> WORKSPACE_CACHE_NAMES = List.of(
            "workspace:byId",
            "workspace:byOwner",
            "workspace:byMember",
            "workspace:public",
    ...
    @Bean
    public ApplicationRunner workspaceCacheWarmupEviction(
            CacheManager cacheManager,
            @Value("${app.cache.clear-workspace-caches-on-startup:true}") boolean clearWorkspaceCachesOnStartup) {
        return args -> {
            if (!clearWorkspaceCachesOnStartup) {
                return;
            }

            WORKSPACE_CACHE_NAMES.forEach(cacheName -> {
                Cache cache = cacheManager.getCache(cacheName);
                if (cache != null) {
                    cache.clear();
                }
            });
            log.info("Cleared workspace Redis caches on startup to remove legacy serialized entries");
        };
    }
}
