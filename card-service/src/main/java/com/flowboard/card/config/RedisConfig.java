...
public class RedisConfig {
    private static final List<String> CARD_CACHE_NAMES = List.of(
            "card:byId",
            "card:byList",
            "card:byBoard",
            "card:byAssignee",
    ...
    @Bean
    public ApplicationRunner cardCacheWarmupEviction(
            CacheManager cacheManager,
            @Value("${app.cache.clear-card-caches-on-startup:true}") boolean clearCardCachesOnStartup) {
        return args -> {
            if (!clearCardCachesOnStartup) {
                return;
            }
            CARD_CACHE_NAMES.forEach(cacheName -> {
                Cache cache = cacheManager.getCache(cacheName);
                if (cache != null) {
                    cache.clear();
                }
            });
            log.info("Cleared card Redis caches on startup to remove legacy serialized entries");
        };
    }
}
