...
public class RedisConfig {
    private static final List<String> BOARD_CACHE_NAMES = List.of(
            "board:byId",
            "board:byWorkspace",
            "board:byMember",
            "board:all",
    ...
    @Bean
    public ApplicationRunner boardCacheWarmupEviction(
            CacheManager cacheManager,
            @Value("${app.cache.clear-board-caches-on-startup:true}") boolean clearBoardCachesOnStartup) {
        return args -> {
            if (!clearBoardCachesOnStartup) {
                return;
            }
            BOARD_CACHE_NAMES.forEach(cacheName -> {
                Cache cache = cacheManager.getCache(cacheName);
                if (cache != null) {
                    cache.clear();
                }
            });
            log.info("Cleared board Redis caches on startup to remove legacy serialized entries");
        };
    }
}
